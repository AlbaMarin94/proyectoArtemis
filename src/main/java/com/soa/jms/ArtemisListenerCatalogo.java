...
public class ArtemisListenerCatalogo {
    ...
    @Value("${pelicula.qry.queue.name.out}")
    public String outQueueName;

    @JmsListener(destination = "${pelicula.qry.queue.name.in}")
    public void receive(String message) {
        ...
        Respuesta respuesta = business.consultarPelicula(pelicula.getNombre());
